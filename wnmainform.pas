...
uses
  ...
  ucmdbox, StdCtrls, Controls, Buttons, Menus, {TAGraph, TASeries ,}
  {TAChartUtils, TASources, TATools, TACustomSeries,}Parse2;
...
begin
 ...
 begin
  ...
  begin
   CmdBox.Clear;
  end else if aux = 'newh' then
  begin
   p:=Pos('(',Input);
   q:=Pos(')',Input);
   h:=StrToFloat(Copy(Input,p+1,q-p-1));
  end else if Input='exit' then close else
  begin
   if (aux = 'raiz')or(aux = 'integral')or(aux = 'edo') then
   begin
     Parsa := TParse.create;
     Parsa.Expression := Input;
     Parsa.ChangeH(h);
     CmdBox.TextColors(clBlack,ClYellow);
     CmdBox.Writeln(FloatToStr(Parsa.Evaluate()));
   end else
   begin
    CmdBox.Writeln('Invalid Command!'+aux);
   end;
  end ;
 end;
